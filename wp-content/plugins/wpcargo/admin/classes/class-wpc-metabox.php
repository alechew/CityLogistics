...
class WPCargo_Metabox {
     ...
     */
	 public function shipment_status_display_callback( $post ){
		...
		<div class="misc-pub-section wpc-status-section" style="background-color: #d4d4d4; border-top: 1px solid #757575;border-bottom: 1px solid #757575;">
			...
			<p><input style="width:100%;" class="date datepicker" type="text" name="status_date" value="<?php echo $wpcargo->user_date(get_current_user_id()); ?>" placeholder="<?php _e( 'yyyy-mm-dd', 'wpcargo' ); ?>"/></p>
			<p><input style="width:100%;" class="time timepicker" type="text" name="status_time" value="<?php echo $wpcargo->user_time(get_current_user_id()); ?>" /></p>
	 ...
	 public function add_metabox() {
    	...
    	add_meta_box(
    		'wpc_add_meta_box',
    		__('WPCargo Shipment Details', 'wpcargo' ),
		...
		add_meta_box(
			...
			__( apply_filters( 'wpc_shipment_history_header', 'WPCargo Shipment History' ), 'wpcargo' ),
		...
		if(!empty($wpc_mp_settings['wpc_mp_enable_admin'])) {
			...
			add_meta_box( 'wpcargo-multiple-package',
				__( apply_filters( 'wpc_multiple_package_header', 'WPCargo Multiple Package' ), 'wpcargo' ),
	...
	public function wpcargo_shipment_history_email_template($post_id){
		...
		if( empty( $_POST['wpcargo_status'] ) ){
			return;
		}
        if( !empty( $wpcargo->mail_status ) ){
        	if( !in_array( $_POST['wpcargo_status'], $wpcargo->mail_status  )  ){
        		return;
        	}
        }
