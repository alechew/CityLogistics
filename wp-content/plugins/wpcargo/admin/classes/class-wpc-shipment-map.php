...
class WPC_Shipment_History_Map{
	...
	function admin_menu(){
		add_submenu_page(
			'wpcargo-settings',
			__( 'Map Settings', WPCARGO_TEXTDOMAIN ),
			__( 'Map Settings', WPCARGO_TEXTDOMAIN ),
			'manage_options',
	...
	function settings_navigation(){
		$view = $_GET['page'];
		?>
		<a class="nav-tab <?php echo ( $view == 'wpc-shmap-settings') ? 'nav-tab-active' : '' ;  ?>" href="<?php echo admin_url().'admin.php?page=wpc-shmap-settings'; ?>" ><?php _e('Map Settings', 'wpcargo'); ?></a>
		<?php
	}
	function map_settings_callback(){
		...
		?>
		<div class="wrap">
        	<h1><?php _e('Map Settings', WPCARGO_TEXTDOMAIN ); ?></h1>
	...
	function gmap_library(){
		...
		//if( $screen->post_type == 'wpcargo_shipment' && $screen->base == 'post' && $shmap_active ){
		if( $screen->post_type == 'wpcargo_shipment' && $shmap_active ){
